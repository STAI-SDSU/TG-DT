...
from tg_dt.text_decision_transformer import TextDecisionTransformer
from tg_dt.text_seq_trainer import TextSequenceTrainer
from tg_dt.text_utils import get_env_list
from tg_dt.text_utils import get_prompt_batch, get_prompt, get_batch, get_batch_finetune
from tg_dt.text_utils import process_total_data_mean, load_data_prompt_text, process_info
from tg_dt.text_utils import eval_episodes
...
def experiment_mix_env(
    ...
    data_save_path = os.path.join('/data/TGDT_data/', 'data')
    save_path = os.path.join(cur_dir, 'model_saved/')
    if not os.path.exists(save_path): os.mkdir(save_path)

    config_path_dict = {
        'cheetah_vel': "cheetah_vel/cheetah_vel.json",
        'cheetah_dir': "cheetah_dir/cheetah_dir.json",
        'ant_dir': "ant_dir/ant_dir.json",
    ...
    trajectories_list, prompt_trajectories_list, description_list, prompt_description_list = \
        load_data_prompt_text(train_env_name_list, data_save_path, dataset_mode, train_prompt_mode, args)
    # load testing dataset
    test_trajectories_list, test_prompt_trajectories_list, test_description_list, test_prompt_description_list= \
        load_data_prompt_text(test_env_name_list, data_save_path, test_dataset_mode, test_prompt_mode, args)
    ...
    model = TextDecisionTransformer(
        state_dim=state_dim, act_dim=act_dim, max_length=K, max_ep_len=1000, device=device, hidden_size=variant['embed_dim'],
        n_layer=variant['n_layer'], n_head=variant['n_head'], n_inner=4 * variant['embed_dim'], activation_function=variant['activation_function'],
    ...
    trainer = TextSequenceTrainer(
        model=model,
        optimizer=optimizer,
        batch_size=batch_size,
        get_batch=get_batch(trajectories_list[0], description_list[0], info[env_name], variant), # TODO
        scheduler=scheduler,
        loss_fn=lambda s_hat, a_hat, r_hat, s, a, r: torch.mean((a_hat - a) ** 2),
        eval_fns=None,
        get_prompt=get_prompt(prompt_trajectories_list[0], prompt_description_list[0], info[env_name], variant),
        get_prompt_batch=get_prompt_batch(trajectories_list, description_list, prompt_trajectories_list, prompt_description_list, info, variant, train_env_name_list),
        device = device
    )
    ...
    if not variant['evaluation']:
        ...
        for iter in range(variant['max_iters']):
            ...
            outputs = trainer.pure_train_iteration_mix(num_steps=variant['num_steps_per_iter'])

            # start evaluation
            if iter % args.test_eval_interval == 0:
                # evaluate test
                if not args.finetune:
                    test_eval_logs = trainer.eval_iteration_multienv( # TODO
                        get_prompt, test_prompt_trajectories_list, test_prompt_description_list, 
                    ...
                    test_eval_logs = trainer.finetune_eval_iteration_multienv(# TODO
                        get_prompt, get_batch_finetune, test_prompt_trajectories_list, test_prompt_description_list, test_trajectories_list,
            ...
            if iter % args.train_eval_interval == 0:
                ...
                train_eval_logs = trainer.eval_iteration_multienv(
                    get_prompt, prompt_trajectories_list, prompt_description_list, 
        ...
        eval_logs = trainer.eval_iteration_multienv(        # TODO
...
if __name__ == '__main__':
    ...
    parser.add_argument('--project', type=str, default='text-dt')
